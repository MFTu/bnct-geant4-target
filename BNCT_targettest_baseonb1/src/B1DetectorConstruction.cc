...
// left side reflector
 ...
 // detector right after neutron conversion target

  G4Material* detector_fortar_mat = nist->FindOrBuildMaterial("G4_POLYSTYRENE"); //POLYSTYRENE
  G4ThreeVector pos_detector_fortar = G4ThreeVector((6.6/2.)*cm, 0, ((0/2.)+(8/2.)+(0)/2.)*cm);

  G4double detector_fortar_innerRadius = 0.*cm; G4double detector_fortar_outerRadius = 10/2.*cm; 
  G4double detector_fortar_hz = 10/2.*cm;
  G4double detector_fortar_startAngle = 0.*deg; G4double detector_fortar_spanningAngle = 360.*deg;
  G4Tubs* solidDetectorforTarget
     = new G4Tubs("DetectorforTarget",
               detector_fortar_innerRadius,
               detector_fortar_outerRadius,
               detector_fortar_hz,
               detector_fortar_startAngle,
               detector_fortar_spanningAngle);   


                      
  G4LogicalVolume* logicDetectorforTarget =                         
    new G4LogicalVolume(solidDetectorforTarget,         //its solid
                        detector_fortar_mat,          //its material
                        "DetectorforTarget");           //its name
  logicDetectorforTarget->SetVisAttributes(new G4VisAttributes(G4Colour::Yellow()));


  G4RotationMatrix* yDetforTarRot = new G4RotationMatrix; // Rotates X and Z axes only 
   yDetforTarRot->rotateY(-M_PI/4.*rad); // Rotates 45 degrees 
               
  new G4PVPlacement(yDetforTarRot,                       //rotation
                    pos_detector_fortar,                    //at position
                    logicDetectorforTarget,             //its logical volume
                    "DetectorforTarget",                //its name
                    logicWorld,                //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
